...
export class KnowledgeSplit extends Component {
    ...
    setup() {
        ...
        // Robust service loading for Odoo 19
        try {
    ...
    async editArticle() {
        ...
        const actionDef = await this.rpc('/web/action/load', {
            action_id: 'mba_knowledge_management.action_knowledge_article_content_only'
        });
    ...
    async archiveArticle() {
        ...
        try {
            ...
            const uid = Number(this.user?.userId || this.session?.uid || this.env.session?.uid || NaN);
            const isOwner = ownerId === uid;
            const isAdmin = this.user?.hasGroup ? await this.user.hasGroup("base.group_system") : false;
    ...
    async _getPartnerId() {
        const uid = this.user?.userId || this.session?.uid || this.env.session?.uid;
        if (!uid) return null;
        const [user] = await this.orm.read("res.users", [uid], ["partner_id"]);
